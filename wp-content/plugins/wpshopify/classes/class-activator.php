...
class Activator
{
   ...
   public function toggle_activation_flags()
   {
      ...
      if ($this->plugin_ready()) {
         ...
         // If Pro is already active, just deactivate free verison
         if ($this->DB_Settings_General->is_pro_tier()) {

            $this->deactivate_free_version();
            $this->DB_Settings_General->set_free_tier(0);
         
         // Lands here if free version
         } else {

            $this->DB_Settings_General->set_free_tier();
            $this->DB_Settings_General->set_free_basename(WP_SHOPIFY_BASENAME);
         }
