...
class Variants extends \WP_Shopify\DB
{
   ...
   public function get_in_stock_variants_from_product($product)
   {
      ...
      return Utils_Sorting::sort_by(Utils::only_available_variants($variants), 'position');
   }
	...
	*/
   public function sort_variants_by_position($variants)
   {
      return Utils_Sorting::sort_by($variants, 'position');
   }
   ...
   public function get_last_variant_compare_at_price($variants, $last_variant_index)
   {
      ...
      $found_last = array_slice($variants, -1)[0];

      if (empty($found_last)) {
         return $variants[$last_variant_index]->compare_at_price;
      }

      return $found_last->compare_at_price;
   }
