...
class Utils
{
   ...
   public static function get_path_from_parts($parts) {
      ...
      if (strpos($parts[0], 'http') !== false || strpos($parts[0], 'https') !== false) {
   ...
   public static function get_abs_admin_path($path = '') {
      ...
      $host_parts = self::get_host_parts($hostname);

      $wp_path = self::get_path_from_parts($host_parts);

      if(function_exists('get_home_path')) {
         $abs_wp_path = rtrim(get_home_path(), '/');

      } else {
         $abs_wp_path = rtrim(ABSPATH, '/');
      }

      $final_path = $abs_wp_path . $wp_path . $path;
