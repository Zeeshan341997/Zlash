...
class Skrill_Payment_Gateway extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		if ( ( isset( $transaction_id ) && isset( $msid ) ) ||
			( isset( $status ) && isset( $status_url ) ) ||
			( isset( $status ) && isset( $refund_status_url ) )
		) {
			add_filter(
	...
	/**
	 * Check if credit card enabled and show separately at backend configuration
	 *
	 * @return bool
	 */
	public function is_credit_card_avaiable() {
		$acc_settings = get_option( 'woocommerce_skrill_acc_settings' );
		if ( 'yes' === $acc_settings['enabled'] && 'yes' === $acc_settings['show_separately'] ) {
			return true;
		}
	...
	public function is_available() {
		...
		if ( $is_available ) {
			if ( $this->is_payment_method_in_credit_card_list && $this->is_credit_card_avaiable() ) {
				return false;
			} elseif ( $this->is_enabled() && $this->is_show_separately() && $this->is_country_allowed() ) {
				return true;
			}
		}
	...
	protected function get_checkout_parameters() {
		...
		$anti_fraud_hash                              = Skrill_Payment::generate_anti_fraud_hash( $this->wc_order->get_total(), $this->wc_order->get_order_number(), $checkout_parameters['transaction_id'] );
	...
	protected function process_status_response() {
		...
		$is_fraud                     = Skrill_Payment::is_fraud( $payment_response['amount'], $payment_response['order_id'], $payment_response['transaction_id'], $payment_response['secure_payment'] );
	...
	protected function change_order_status() {
		...
		if ( $original_post_status === $this->wc_pending_status &&
			...
			( $original_post_status === $this->wc_processing_status
			...
			( $original_post_status === $this->wc_processing_status || $original_post_status === $this->wc_completed_status )
			&& ( $transaction['payment_status'] === $this->skrill_processed_status
				|| $transaction['payment_status'] === $this->skrill_refund_failed_status )
			&& $order_post_status === $this->wc_refunded_status
		) {
